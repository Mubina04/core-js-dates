...
function getTime(date) {
  ...
  const minute = dateN.getMinutes();
  const second = dateN.getSeconds();
...
function getNextFriday(date) {
  ...
  givenDate.setUTCDate(givenDate.getUTCDate() + daysUntilFriday);
  return givenDate;
}
...
function formatDate(date) {
  ...
  return (
    ...
    `${d.getUTCHours() % 12 || 12}:${String(d.getUTCMinutes()).padStart(2, '0')}:` +
    `${String(d.getUTCSeconds()).padStart(2, '0')} ${d.getUTCHours() < 12 ? 'AM' : 'PM'}`
  );
}
...
function getWeekNumberByDate(date) {
  ...
  const daysSinceYearStart = Math.floor(
    (tempDate - startOfYear) / (1000 * 60 * 60 * 24)
  );
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    ...
    cyclePosition += 1;
    if (cyclePosition >= countWorkDays + countOffDays) {
      cyclePosition = 0;
    }
